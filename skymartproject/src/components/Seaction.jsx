    ...
    <section className="w-full bg-[#0b0b0b]">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-3 md:grid-cols-2 gap-7 p-9">
        {features.map((item) => (
          <div
            key={item.id}
            className="group relative overflow-hidden rounded-2xl border border-zinc-700 bg-[#121212] p-7 transition-all duration-500 hover:-translate-y-2 hover:border-lime-400 hover:shadow-[0_15px_40px_rgba(163,230,53,.15)]"
          >
            {/* Glow Effect */}
            ...
            <div className="flex items-center gap-5 relative z-10">
              <div
                className={`w-16 h-16 rounded-xl bg-[#1b1b1b] border border-zinc-700 flex items-center justify-center ${item.color} ${item.bg} transition-all duration-500 group-hover:scale-110`}
              >
                {item.icon}
              </div>

              <div>
                <h2 className="text-white text-xl font-semibold group-hover:text-lime-400 transition">
                  {item.title}
                </h2>
