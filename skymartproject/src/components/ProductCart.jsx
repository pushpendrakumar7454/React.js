...
const ProductCart = ({ product }) => {
  ...
  return (
    <div className="group relative overflow-hidden cursor-pointer rounded-2xl lg:rounded-[28px] border border-zinc-800 dark:bg-zinc-900 transition-all duration-500 hover:-translate-y-2 hover:border-violet-500/50 hover:shadow-[0_20px_60px_rgba(139,92,246,.25)]">
      ...
      >
        {/* Category */}
        <span className="absolute left-2 top-2 lg:left-4 lg:top-4 z-20 rounded-full dark:bg-white/10 px-2 py-1 lg:px-3 text-[10px] sm:text-xs font-semibold text-white backdrop-blur-md">
          Electronics
        </span>
        ...
        <div className="absolute right-2 top-2 lg:right-4 lg:top-4 z-20 flex flex-col gap-2 opacity-100 lg:opacity-0 transition-all duration-300 lg:group-hover:opacity-100">
          <button className="flex h-8 w-8 lg:h-10 lg:w-10 items-center justify-center rounded-full bg-white dark:text-black shadow-lg transition hover:scale-110">
            <Heart size={16} className="lg:w-[18px] lg:h-[18px]" />
          </button>

          <button className="flex h-8 w-8 lg:h-10 lg:w-10 items-center justify-center rounded-full dark:bg-white text-black shadow-lg transition hover:scale-110">
        ...
        {/* Gradient */}
        <div className="absolute inset-0 dark:bg-gradient-to-t from-black via-transparent to-transparent opacity-70"></div>
      </div>

      {/* Content */}
      <div className="space-y-3 lg:space-y-4 p-3 lg:p-4">
        <div>
          <p className="text-[10px] sm:text-xs lg:text-sm uppercase tracking-widest dark:text-zinc-500">
            Premium Audio
          </p>

          <h2 className="mt-1 text-sm sm:text-base lg:text-lg font-semibold dark:text-white transition group-hover:text-violet-400 leading-snug">
        ...
        {/* Price */}
        <div className="flex items-center justify-between gap-2 lg:gap-16">
          <div>
            <h3 className="text-lg sm:text-xl lg:text-2xl font-semibold dark:text-white">
              ${product.price}
            </h3>
          </div>
