...
const Furniture = () => {
  ...
  const categories = useMemo(() => {
    const list = [...new Set(furnicureData.map((item) => item.category))];
    return ["All Categories", ...list];
  }, [furnicureData]);

  // Search + Category Filter
  let filteredProducts = furnicureData.filter((product) => {
    const matchSearch =
      ...
      <div className="flex flex-col gap-4 lg:flex-row">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="p-3 mt-0.5 w-full rounded-2xl border border-lime-700 dark:bg-zinc-900 pl-14 pr-4 dark:text-white outline-none placeholder:text-zinc-500 focus:border-violet-500"
          />
        </div>
