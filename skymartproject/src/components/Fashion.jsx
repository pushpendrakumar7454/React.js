...
const Fashion = () => {
  const { fashionData, setFashionData } = useContext(MyStore);
  ...
  const categories = useMemo(() => {
    const list = [...new Set(fashionData.map((item) => item.category))];
    return ["All Categories", ...list];
  }, [fashionData]);

  let filteredProducts = fashionData.filter((product) => {
    const matchSearch =
      ...
      <div className="flex flex-col gap-4 lg:flex-row">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="p-3 mt-1 w-full rounded-2xl border border-lime-700 dark:bg-zinc-900 pl-14 pr-4 dark:text-white outline-none placeholder:text-zinc-500 focus:border-violet-500"
          />
        </div>
